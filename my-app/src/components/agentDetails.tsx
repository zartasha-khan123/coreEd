...
// Vimeo ke "external/370855368.sd.mp4" ya "video/370855368" se id nikaalo
...
export default function AgentDetailsPage({ agentId  }: AgentDetailsPageProps) {
...
{agent.videos?.length > 0 && (
  <section className="mx-auto max-w-7xl px-4 md:px-6 mt-10">
    ...
    <div className="relative mt-6">
      {/* Gradient overlays */}
      <div className="pointer-events-none absolute left-0 top-0 h-full w-6 sm:w-10 bg-gradient-to-r from-transparent to-[#0b0616]" />
      <div className="pointer-events-none absolute right-0 top-0 h-full w-6 sm:w-10 bg-gradient-to-l from-transparent to-[#0b0616]" />

      {/* Cards container */}
      <div className="flex flex-wrap justify-center gap-4">
        {agent.videos.map(
          (
            video: { thumbnail: string; url: string; title?: string; duration?: string },
            index: number
          ) => (
            <Card
              key={index}
              className={`w-full sm:w-[15rem] md:w-[18rem] overflow-hidden transition-all hover:shadow-lg ${glass} ${purpleRing}`}
            >
              <CardContent className="p-0">
                <button
                ...
                >
                  <Image
                    ...
                    width={288} // 18rem
                    height={162} // 16:9 ratio
                    className="object-cover w-full h-full rounded-t"
                  />
                  {/* Play overlay */}
                  <div className="absolute inset-0 bg-black/30 opacity-0 hover:opacity-100 transition grid place-items-center rounded-t">
                    <PlayCircle className="h-12 w-12 text-white drop-shadow" />
                  </div>
                  {/* Duration */}
                  {video.duration && (
    ...
    <div className="relative w-full max-w-4xl">
      ...
      {isYouTube(selectedVideo) ? (
        <iframe
          ...
          src={`https://www.youtube.com/embed/${ytId(selectedVideo)}?autoplay=1&mute=1&rel=0`}
          title="YouTube video player"
          allow="autoplay; encrypted-media; picture-in-picture"
          allowFullScreen
        />
      ) : isVimeo(selectedVideo) ? (
        <iframe
          key={selectedVideo}
          className="w-full aspect-video rounded-xl"
          src={`https://player.vimeo.com/video/${vimeoId(selectedVideo)}?autoplay=1&muted=1`}
          title="Vimeo video player"
          allow="autoplay; fullscreen; picture-in-picture"
          allowFullScreen
        />
      ) : isDirectMP4(selectedVideo) ? (
        <video
          key={selectedVideo}
        ...
        <div className="aspect-video w-full grid place-items-center rounded-xl bg-white/5 text-white/80">
          Unable to play this URL.
        </div>
      )}
    </div>
  </div>
